  ...
  x0, double resamp_data[57820])
{
  ...
  for (int k = 0; k < i; k++) {
    ...
    resamp_data[k] = ((b_a3_tmp - resamp_len * d) + (((a3_tmp - b_a3_tmp) - a3)
      - resamp_len) * (d * d)) - a3 * rt_powd_snf(d, 3.0);
  }
}
    ...
    *shift_for_qam_data = 0.0;
    ...
    if (flag_array_zero == 1) {
      ...
      *shift_for_qam_data = 0.0;
      ...
      // 280373;
      // %%240MHz / 131
      coder::rat(f_opt * 280000.0 / f_sine / Fs, &kd, &meanPhase);
      lagrange_resamp(sweep_data, kd, meanPhase, 0.0, resamp_sweep);
      kd = coder::internal::b_maximum(resamp_sweep);
      ...
      *sweep_warning_status = 0.0;
      ...
      for (i = 0; i < b_i; i++) {
        meanPhase = d + static_cast<double>(i);
        found_c_max[static_cast<int>(((meanPhase - 57820.0) + PreSPP * 2.0) +
          1.0) - 1] = c[static_cast<int>(meanPhase) - 1];
      }
      ...
      if (iindx >= PreSPP * 2.0) {
        ...
        *shift_for_qam_data = 0.0;
        ...
        for (b_i = 0; b_i < 56001; b_i++) {
          ...
          if (bi == 0.0) {
            ...
            if (brm > kd) {
              meanPhase = bi / br;
              kd = br + meanPhase * bi;
              re = (ar + meanPhase * ai) / kd;
              meanPhase = (ai - meanPhase * ar) / kd;
            } else if (kd == brm) {
              if (br > 0.0) {
                meanPhase = 0.5;
              } else {
              ...
              re = (ar * meanPhase + ai * kd) / brm;
              meanPhase = (ai * meanPhase - ar * kd) / brm;
            } else {
              meanPhase = br / bi;
              kd = bi + meanPhase * br;
              re = (meanPhase * ar + ai) / kd;
              meanPhase = (meanPhase * ai - ar) / kd;
            }
          }

          TrFunEst[b_i].re = re;
          TrFunEst[b_i].im = meanPhase;
        }

        coder::fftshift(TrFunEst);
        for (k = 0; k < 14001; k++) {
          bi = TrFunEst[k + 28001].im;
          meanPhase = TrFunEst[k + 28001].re;
          newPhase[k] = rt_atan2d_snf(bi, meanPhase);
          newGain[k] = rt_hypotd_snf(meanPhase, bi);
        }
        ...
        for (i = 0; i < 14000; i++) {
          bi = newPhase[i + 1];
          meanPhase = bi - newPhase[i];
          if (meanPhase > std::floor((kd - 1.0) * 2.0 * 3.1415926535897931) -
              0.1) {
            if (meanPhase > std::floor(kd * 2.0 * 3.1415926535897931) - 0.1) {
              kd++;
            }
        ...
        for (b_i = 0; b_i < 513; b_i++) {
          dv1[b_i] = 136.71875 * static_cast<double>(b_i);
        }

        coder::vmovfun(newGain, 14001, b_dv);
        coder::interp1(*(double (*)[14001])&F_tr[28001], b_dv, dv1, gain_s);
        ...
        for (k = 0; k < 57820; k++) {
          ...
          re = meanPhase * 0.0;
          channel2[k].re = re;
          channel2[k].im = meanPhase;
          if (meanPhase == 0.0) {
            channel2[k].re = std::exp(re);
            channel2[k].im = 0.0;
          } else {
            kd = std::exp(re / 2.0);
            channel2[k].re = kd * (kd * std::cos(meanPhase));
            channel2[k].im = kd * (kd * std::sin(meanPhase));
          }
        ...
        for (b_i = 0; b_i < 57820; b_i++) {
          ...
          bi = channel2[b_i].im;
          meanPhase = ft[b_i];
          br = channel2[b_i].re + bi * 0.0 * meanPhase;
          bi *= meanPhase;
          if (bi == 0.0) {
            if (ai == 0.0) {
              re = ar / br;
