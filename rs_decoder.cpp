...
//{
//    // tt = K1 - number of errors that can be corrected
//    // kk = T1 - number of data bytes in block

//    int i,j,u,q;
//    int elp[nn-K1+2][nn-K1], d[nn-K1+2], l[nn-K1+2], u_lu[nn-K1+2], s[nn-K1+1] ;
//    int count=0, syn_error=0, root[T1], loc[T1], z[T1+1], err[nn], reg[T1+1] ;

//    /* recd[] is in polynomial form, convert to index form */
//    for (i=0; i < nn; i++)
//        recd[i] = index_of[recd[i]];

//    /* first form the syndromes; i.e., evaluate recd(x) at roots of g(x) namely @**(b0+i), i = 0, ... ,(2*tt-1) */
//    for (i=1; i <= nn-K1; i++)
//    {
//        s[i] = 0 ;
//        for (j=0; j < nn; j++)
...
//            if (u<nn-K1)    /* no discrepancy computed on last iteration */
//            {
//                if (s[u+1]!=-1)
...
//            }
//        } while ((u < nn-K1) && (l[u+1] <= T1)) ;


//        u++ ;
//        if (l[u] <= T1)         /* can correct error */
//        {
